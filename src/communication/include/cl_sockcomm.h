/*
 * File:  cl_sockcomm.h                          Version: 001
...
#ifndef __NPS_CL_SOCKCOMM__
#define __NPS_CL_SOCKCOMM__

#include "sockcomm.h"

class CL_SockCommClass : public virtual SockCommClass {
// Connection-Less Socket Communication Class

public:

   CL_SockCommClass ();
   virtual ~CL_SockCommClass ();
...
protected:
   ...
   int read_from_socket ( char * /*buffer*/,
                          ...
                          const unsigned int /*buffer length*/,
                          int & /*error_num*/ );
      // Reads up to "buffer length" bytes from the socket into the 
      // "buffer" (i.e. "buffer length" should be preset to the maximum
      // size of the "buffer".)  On error -1 is returned and error_num
