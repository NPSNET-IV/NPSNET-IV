/*
 * File:  bc_ip_cl_sockcomm.cc                   Version: 001
...
BC_IP_CL_SockCommClass::BC_IP_CL_SockCommClass ( const unsigned short port,
                                                 const char *interface,
                                                 const int loopback )
   : IP_CL_SockCommClass ( port, interface, loopback, IFF_BROADCAST )
{
   if ( sockstate != SC_SOCKET_ERROR ) {
      // Assign network port
      if ( port == 0 ) {
         net_port = DEFAULT_BC_PORT;
      }
      else {
         net_port = port;
      }
...
BC_IP_CL_SockCommClass::~BC_IP_CL_SockCommClass ()
{
}


int
BC_IP_CL_SockCommClass::configure_socket ( int &error_num )
{
   int success = TRUE;
   error_num = 0;
   int on = 1;
   ...
   if ( sockop == SC_SOCKET_WRITE ) {
      ...
      if ( success ) {
         ...
         memcpy((void *) &(internet_sockaddr.sin_addr),
                (void *) &(broadaddr.sin_addr), 
                (size_t)IN_ADDR_SIZE);
      }
      ...
      internet_sockaddr.sin_family = AF_INET;
      internet_sockaddr.sin_addr.s_addr = broadaddr.sin_addr.s_addr;
      internet_sockaddr.sin_port = htons(net_port);
...
int
BC_IP_CL_SockCommClass::filter ( sockaddr *from )
{
   sockaddr_in *from_in = (sockaddr_in *)from;
   if ( !net_loopback &&
        ((memcmp((void *) &(from_in->sin_addr),
                 (void *) &(net_inet_addr),
...
int
BC_IP_CL_SockCommClass::isa ( SockKind query_kind )
{
   static int truth_value;
   truth_value = ( BC_IP_CL_SockCommClass::isaexact(query_kind) ||
...
int
BC_IP_CL_SockCommClass::isaexact ( SockKind query_kind )
{
   return (query_kind == SC_UDP_BROADCAST);
}


// End file - bc_ip_cl_sockcomm.cc
