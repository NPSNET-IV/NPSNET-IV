...
void setupwin( long *win )
{
   ...
   *win = winopen ("Sound Server Window");
   minsize(640,480);
   keepaspect(1279,1023);
   winconstraints();
   RGBmode ();
...
int update_window ( STATE_REC &state )
   ...
   strcpy ( text, "A DIS PDU Based Mono Audio Server" );
   ...
   fmsetfont(littlefont);
   strcpy ( text, "Written by: Paul T. Barham and John T. Roesli" );
   ...
   fmprstr(text);
   strcpy ( text, "Dr. Robert McGhee & Mr. John Falby" );
...
*/
   ...
   switch ( state.mode )
      {
      case loading:
         strcpy ( text, "Initializing and Loading sound files ..." );
         break;
      case no_vehicle:
...
/* PRINT TEXT STUFF */
   ...
   sprintf ( text, "NPS Mono Audio Server" );
   ...
   sprintf ( text, "Current host: %s", master.hostname );
   ...
   switch ( state.mode )
      {
      case loading:
         strcpy ( text, "Loading sound files ..." );
         break;
      case no_vehicle:
