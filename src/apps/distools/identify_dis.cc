...
static InitManager *initman;
static IDENTIFY *identify = NULL;
...
void output_Entity_Info ( ID_ENTITY_REC &entity_info ) 
   {
   cout << "\tEntity identified, midi_veh_snd = " << entity_info.midi_veh_snd << endl;
   }


void output_Fire_Info ( EFFECT_REC &effect )
   {
   cout << "\tFire Effect: " << effect.fire_effect << endl;
   cout << "\tSmoke Effect: " << effect.smoke_effect << endl;
   cout << "\tFire Local Sound: " << effect.fire_local_snd << endl;
   cout << "\tFire Midi Sound: " << effect.fire_midi_snd << endl;
   }

void output_Detonation_info ( EFFECT_REC &effect )
   {
   cout << "\tDetonation Effect: " << effect.det_effect << endl;
   cout << "\tSmoke Effect: " << effect.smoke_effect << endl;
   cout << "\tDetonation Local Sound: " << effect.det_local_snd << endl;
   cout << "\tDetonation Midi Sound: " << effect.det_midi_snd << endl;
   }
...
void output_EntityStatePDU ( EntityStatePDU *es, sender_info sender,
   ...
   if ( identify->entity(&(es->entity_type),&entity_info) ) {
      output_Entity_Info(entity_info);
   }
   else {
      cout << "\tWARNING:\tCannot identify entity information." << endl;
   }
...
void output_FirePDU ( FirePDU *fire, sender_info sender, char *leader )
   ...
   if ( identify->effect(&(fire->burst_descriptor.munition), &effect) ) {
      output_Fire_Info ( effect );
   }
   else {
      cout << "\tWARNING:\tCannot identify fire information." << endl;
   }
...
void output_DetonationPDU ( DetonationPDU *det, sender_info sender, 
   ...
   if ( identify->effect(&(det->burst_descriptor.munition), &effect) ) {
      output_Detonation_info ( effect );
   }
   else {
      cout << "\tWARNING:\tCannot identify detonation information." << endl;
   }
...
main ( int argc, char **argv )
   ...
   strncpy ( identify_file, local.string, FILENAME_SIZE );
   ...
   if ( (strcmp(identify_file,"NONE")) != 0 ) {
      identify = new IDENTIFY(identify_file);
      cerr << "Identify File is: " << identify_file << endl << endl;
   }
